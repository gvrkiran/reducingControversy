		...
		if(attr=="Retweets") {
			...
			for (var i = 0; i < arrayLength; i++) { // take the top 3 random retweets
				if(d_shuffled[i]!="") {
					...
					tmp_str += "  <a id='show_link" + i.toString() + "' href='#' onclick='$(\"#manual-example" + i.toString() + " a[rel=tipsy]\").tipsy(\"show\"); hideDivShow(" + i.toString() + "); return false;'>Show</a><a id='hide_link" + i.toString() + "' href='#' onclick='$(\"#manual-example" + i.toString() + " a[rel=tipsy]\").tipsy(\"hide\"); hideDivHide(" + i.toString() + "); return false;'></a>";
